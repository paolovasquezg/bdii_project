"""
ISAM • CSV + SQL smoke test
- Crea tabla con PRIMARY KEY USING isam
...
def main():
    tbl = "isam_products_sql"
    csv_path = os.path.join(HERE, "_testdata", "csv", "products.csv")
    ensure_csv(csv_path)

    print_section("ISAM: create table + import csv")
    run_sql(f"DROP TABLE IF EXISTS {tbl};")
    run_sql(f"""
        CREATE TABLE {tbl} (
            product_id INT PRIMARY KEY USING isam,
            name VARCHAR(32),
            price FLOAT,
            stock INT
        );
    ...
    print_section("ISAM: consultas en PK")
    run_sql(f"SELECT * FROM {tbl} WHERE product_id = 4;")
    run_sql(f"SELECT * FROM {tbl} WHERE product_id BETWEEN 1 AND 4;")
    print("\n✅ ISAM test completed.")
